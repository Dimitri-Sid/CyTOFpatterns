...
library(lubridate)
library(dyno)
setwd("/OneDrive - Johns Hopkins/CYTOF_Metrics_Paper/figures")
```
...
3) Nivo in viral and non viral HCC (Hopkins internal) 
...
hiv_cds <- cluster_cells(hiv_cds,resolution = 1e-6)
pData(hiv_cds)$clusters<- hiv_cds@clusters$UMAP$clusters
plot_cells(hiv_cds,group_cells_by = "cluster", color_cells_by  = "cluster")
plot_cells(hiv_cds,group_cells_by = "cluster", color_cells_by  = "cluster", show_trajectory_graph = FALSE, label_cell_groups = FALSE)

marker_test_res <- top_markers(hiv_cds, group_cells_by="cluster", 
...
plot_genes_by_group(hiv_cds[-c(21,23),],
                    top_specific_marker_ids[-c(21,23)],
...
sum <- table(pData(hiv_cds)$clusters, pData(hiv_cds)$condition)
total<-table(pData(hiv_cds)$condition)
...
colnames(sumtable_ptot)<-c("Cluster", "Condition","Proportion")
sumtable_ptot$Condition <- factor(sumtable_ptot$Condition,levels = c("Healthy", "ART", "Intermediate", "Mild","Severe"))
#Barplot
ggplot(sumtable_ptot, aes(fill=factor(Cluster), y=Proportion, x=Condition)) + 
  geom_bar(position="stack", stat="identity")+ggtitle("Cell Type Abundance") +labs(fill = "Group")+theme_minimal()

### COMPOSITION PROFILE by patient
#### Composition analysis for broad cell types 
sum <- table(pData(hiv_cds)$clusters, pData(hiv_cds)$sample_id)
total<-table(pData(hiv_cds)$sample_id )
# Normalize cell counts to total number of cells per treatment group
sumtable_ptot <- t(apply(sum, 1, function(x) x/total))  
sumtable_ptot <- melt(sumtable_ptot)
colnames(sumtable_ptot)<-c("Cluster", "sample_id","Proportion")
#sumtable_ptot$Condition <- factor(sumtable_ptot$Condition,levels = c("Healthy", "ART", "Intermediate", "Mild","Severe"))
#Barplot
sumtable_ptot$condition <- pData(hiv_cds)$condition[match(sumtable_ptot$sample_id,pData(hiv_cds)$sample_id)]

sumtable_ptot$condition <- factor(sumtable_ptot$condition, levels = c("Healthy","ART","Mild","Intermediate","Severe"))
df <- sumtable_ptot[sumtable_ptot$Cluster == 1,]
ggplot(df, aes(x=condition, y=Proportion    ,color = condition))+geom_boxplot()+geom_jitter(show.legend = T)+theme_classic()+ theme(axis.text.x = element_text(size=8, angle = 45, hjust=1, vjust=1, color="black"),axis.title.x = element_blank(),axis.ticks=element_line(color="black"),axis.line.x = element_line(size=0.5), axis.line.y = element_line(size=0.5), axis.text.y = element_text(color="black"), strip.background = element_rect(fill=NA), strip.text = element_text(size=10), panel.background = element_rect(fill="white"),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.title = element_blank(),legend.key.size = unit(1,'lines'),legend.text = element_text(size=10),legend.key = element_rect(fill="white"))+ stat_compare_means(aes(label = ..p.adj..),method="t.test",paired = FALSE,comparisons = combn(unique(as.character(df$condition)),2, simplify = FALSE),hide.ns=FALSE)+ylab("Proportion") + ggtitle("Cluster 1")

df <- sumtable_ptot[sumtable_ptot$Cluster == 2,]
ggplot(df, aes(x=condition, y=Proportion    ,color = condition))+geom_boxplot()+geom_jitter(show.legend = T)+theme_classic()+ theme(axis.text.x = element_text(size=8, angle = 45, hjust=1, vjust=1, color="black"),axis.title.x = element_blank(),axis.ticks=element_line(color="black"),axis.line.x = element_line(size=0.5), axis.line.y = element_line(size=0.5), axis.text.y = element_text(color="black"), strip.background = element_rect(fill=NA), strip.text = element_text(size=10), panel.background = element_rect(fill="white"),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.title = element_blank(),legend.key.size = unit(1,'lines'),legend.text = element_text(size=10),legend.key = element_rect(fill="white"))+ stat_compare_means(aes(label = ..p.adj..),method="t.test",paired = FALSE,comparisons = combn(unique(as.character(df$condition)),2, simplify = FALSE),hide.ns=FALSE)+ylab("Proportion") + ggtitle("Cluster 2")
  
df <- sumtable_ptot[sumtable_ptot$Cluster == 3,]
ggplot(df, aes(x=condition, y=Proportion    ,color = condition))+geom_boxplot()+geom_jitter(show.legend = T)+theme_classic()+ theme(axis.text.x = element_text(size=8, angle = 45, hjust=1, vjust=1, color="black"),axis.title.x = element_blank(),axis.ticks=element_line(color="black"),axis.line.x = element_line(size=0.5), axis.line.y = element_line(size=0.5), axis.text.y = element_text(color="black"), strip.background = element_rect(fill=NA), strip.text = element_text(size=10), panel.background = element_rect(fill="white"),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.title = element_blank(),legend.key.size = unit(1,'lines'),legend.text = element_text(size=10),legend.key = element_rect(fill="white"))+ stat_compare_means(aes(label = ..p.adj..),method="t.test",paired = FALSE,comparisons = combn(unique(as.character(df$condition)),2, simplify = FALSE),hide.ns=FALSE)+ylab("Proportion") + ggtitle("Cluster 3")


df <- sumtable_ptot[sumtable_ptot$Cluster == 2,]
my_comparisons <- combn(unique(as.character(df$condition)),2, simplify = FALSE)
t_test_tooth <- compare_means(Proportion ~condition, comparisons = my_comparisons, p.adjust.method = "fdr", method='t.test', data = df)
t_test_tooth <- t_test_tooth %>% mutate(y.position = rep(max(df$Proportion)+0.2, 10))
ggplot(df, aes(x=condition, y=Proportion    ,color = condition))+geom_boxplot()+geom_jitter(show.legend = T)+theme_classic()+ theme(axis.text.x = element_text(size=8, angle = 45, hjust=1, vjust=1, color="black"),axis.title.x = element_blank(),axis.ticks=element_line(color="black"),axis.line.x = element_line(size=0.5), axis.line.y = element_line(size=0.5), axis.text.y = element_text(color="black"), strip.background = element_rect(fill=NA), strip.text = element_text(size=10), panel.background = element_rect(fill="white"),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.title = element_blank(),legend.key.size = unit(1,'lines'),legend.text = element_text(size=10),legend.key = element_rect(fill="white")) + ggtitle("Cluster 2")+  stat_pvalue_manual(t_test_tooth, label = "p.signif", step.increase = 0.2)

  
save(hiv_cds,file = "hiv_cds_with_clustering.rda")
...
ggplot(df, aes(x=condition, y=pseudotime,color = condition))+geom_boxplot()+geom_jitter(show.legend = T)+theme_classic()+ theme(axis.text.x = element_text(size=8, angle = 45, hjust=1, vjust=1, color="black"),axis.title.x = element_blank(),axis.ticks=element_line(color="black"),axis.line.x = element_line(size=0.5), axis.line.y = element_line(size=0.5), axis.text.y = element_text(color="black"), strip.background = element_rect(fill=NA), strip.text = element_text(size=10), panel.background = element_rect(fill="white"),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.title = element_blank(),legend.key.size = unit(1,'lines'),legend.text = element_text(size=10),legend.key = element_rect(fill="white"))+ stat_compare_means(aes(label = ..p.adj..),method="t.test",paired = FALSE,comparisons = combn(unique(as.character(df$condition)),2, simplify = FALSE),hide.ns=FALSE)+ylab("Pseudotime Pattern Weight Mean")
...
#### 3) HCC
```{r}
load("C:/Users/OneDrive - Johns Hopkins/WON_RESEARCH/CYTOF_Metrics_Paper/paper_CDS_objects/HCC_cds.rda")
setwd("C:/Users/OneDrive - Johns Hopkins/WON_RESEARCH/CYTOF_Metrics_Paper/figures/2_hcc_analysis")
...
HCCdata <- read.table("C:/Users/OneDrive - Johns Hopkins/WON_RESEARCH/CYTOF_Metrics_Paper/metrics/HCCdata_processed.txt")
...
load("./paper_CDS_objects/hodi_Ipi_unstim_projected.rda")
